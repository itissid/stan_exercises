...
\vspace{0.25in}
The original bowling model is simple we have 2 frames in one go. We knock down as many pins as we can in the first frame and then if some are left we knock down some more. We are given some data of a sequence of frames(called a roll). We are given the PMF of the dropping X pins as follows:

$$ P(X=x|\gamma, n) = \frac{F(\gamma+x)}{F(n+\gamma+2) * F(\gamma+1)} $$

Where gamma can be a constant or can come from some distribution. X $\in$ 1:10 
...
5. When gamma is not constant, how do I model it?
...
```{r exposeStanFunctions, cache.path="cache/", tidy = TRUE, message = FALSE, warning = FALSE, error = FALSE, cache=TRUE}
...
```{r stan, cache.path ="cache/", tidy = TRUE, message = FALSE, warning = FALSE, error = FALSE, cache=TRUE}
...
```{r plotPosterior, cache.path ="cache/", tidy=TRUE, message = FALSE, warning = FALSE, error = FALSE, cache=TRUE, dependson="stan"}
