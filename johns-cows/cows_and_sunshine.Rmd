...
1. I tried centering just the X's and that seemed to produce divergent transactions. I had very tight priors on the slope and the beta was almost zero:
```{r eval=FALSE}

    target += uniform_lpdf(sigma|0,1000); // modelling a unknown variance, t should be ok.
    target += uniform_lpdf(a|-1000, 1000); // a ~ normal(0,1)
...
2. Of course it was ~ 0 the unit change in X will still cause large changes in Y, so the prior needs to be much loser. I can only set those kind of limited priors on X if the range of Y is also controlled:

```{r eval=FALSE}
    target += uniform_lpdf(sigma|0,1000); // modelling a unknown variance, t should be ok.
    target += uniform_lpdf(a|-1000, 1000); // a ~ normal(0,1)
    target += normal_lpdf(b|0, 100); // b ~ normal(0,1) -> Remember that the units of this need to be based on Y. Again best is to standardize both
         mean se_mean    sd    2.5%     25%     50%     75%   97.5% n_eff
...
3. Once we standardize both the variables the lines sort of agree with the linear model from earlier. Though the uncertainity is pretty large.
```{r eval=FALSE}
